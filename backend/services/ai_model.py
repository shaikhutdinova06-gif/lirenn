            ...
            "messages": [
                {
                    ...
                    "content": [
                        {
                            "type": "text",
                            "text": """ОЧЕНЬ ВАЖНО: ПЕРВЫМ ДЕЛОМ ОТВЕТЬ: ЭТО ПОЧВА? НАПИШИ "ДА" ИЛИ "НЕТ" В НАЧАЛЕ ОТВЕТА.

Если НЕТ - напиши ТОЛЬКО "НЕТ" и ничего больше. Не продолжай анализ.

Это НЕ почва, если на изображении:
- Животные (кошки, собаки, птицы, люди)
- Мебель, предметы быта
- Еда, продукты питания
- Здания, машины, техника
- Одежда, ткани
- Любые другие неестественные объекты

Это ПОЧВА, если на изображении:
- Земля, грунт, песок, глина
- Образцы почвы в контейнерах
- Почвенные разрезы
- Растения в почве

Если ДА - продолжай анализ:

Определи тип почвы ИЗ ЭТОГО СПИСКА:
- Пустынно-арктические почвы
...
- Желтоземы
- Красноземы
- И другие типы из классификации почв Евразиатской полярной области

Если тип почвы не совпадает точно с названием из списка, выбери наиболее похожий тип.

Определи:
1. Тип почвы (из списка)
2. Цвет
3. Структуру
4. Плотность
5. Особенности
