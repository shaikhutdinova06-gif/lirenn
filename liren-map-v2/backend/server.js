...
app.get("/soil-zones", async (req, res) => {
  try {
    // Try setting SRID to EPSG:3011 (Pulkovo 1995 / 3-degree Gauss-Kruger zone 6) and transform to EPSG:4326
    const r = await pool.query(`
      SELECT id, zone_type, color,
             ST_AsGeoJSON(ST_Transform(ST_SetSRID(geom, 3011), 4326)) as geom
      FROM soil_zones
      LIMIT 1
    `);
